  ...
  const handleImageFileChange = (e) => {
    ...
    if (file) {
      if (file.size > 5 * 1024 * 1024) {
        addToast('Please select an image under 5MB', 'error');
        return;
      }
      const reader = new FileReader();
      reader.onload = () => {
        setFormData(prev => ({ ...prev, image: reader.result }));
        addToast('Food item photo uploaded successfully!', 'success');
      };
      reader.onerror = () => {
            ...
            <form onSubmit={handleAddSubmit} className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <div
                  ...
                  className="group relative cursor-pointer border-2 border-dashed border-slate-200 hover:border-emerald-500 rounded-2xl p-3 bg-slate-50/70 hover:bg-emerald-50/30 transition-all flex flex-col sm:flex-row items-center gap-3.5"
                >
                  {formData.image ? (
                    <div className="relative w-full sm:w-28 h-24 rounded-xl overflow-hidden shrink-0 border border-slate-200 bg-slate-100 shadow-sm">
                      <img
                        src={formData.image}
                        alt="Surplus preview"
                  ...
                  <div className="flex-1 text-center sm:text-left space-y-1">
                    <div className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-white border border-slate-200 text-xs font-bold text-slate-800 shadow-xs group-hover:border-emerald-300">
                      <Upload className="w-3.5 h-3.5 text-emerald-600" />
                      <span>Upload photo from device</span>
                    </div>
                    <p className="text-[11px] text-slate-500">
                      Take a photo or choose from files (PNG, JPG, WebP up to 5MB)
                    </p>
                  </div>
                </div>
                ...
                {showUrlInput && (
                  <div className="pt-1">
                    <input
                      type="url"
                      placeholder="https://images.unsplash.com/your-item.jpg"
                ...
                <div className="flex items-center gap-1.5 overflow-x-auto pt-1 pb-1">
                  ...
                  {[
                    { label: 'Samosa / Snacks', url: 'https://images.unsplash.com/photo-1601050690597-df0568f70950?auto=format&fit=crop&w=800&q=80' },
                    { label: 'Thali / Meals', url: 'https://images.unsplash.com/photo-1585937421612-70a008356fbe?auto=format&fit=crop&w=800&q=80' },
                    { label: 'Bakery Bread', url: 'https://images.unsplash.com/photo-1509440159596-0249088772ff?auto=format&fit=crop&w=800&q=80' },
                    { label: 'Biryani Box', url: 'https://images.unsplash.com/photo-1563379091339-03b21ab4a4f8?auto=format&fit=crop&w=800&q=80' },
                    { label: 'Fresh Fruits', url: 'https://images.unsplash.com/photo-1610348725531-843dff563e2c?auto=format&fit=crop&w=800&q=80' },
                  ].map((preset) => (
                    <button
                      type="button"
                      key={preset.label}
                      onClick={() => {
                        ...
                        addToast(`Applied ${preset.label} photo`, 'info');
                      }}
