            ...
            {isAuthenticated && currentUser ? (
              <div className="relative" ref={userRef}>
                <button
                  ...
                  className="flex items-center gap-2 pl-2 pr-3 py-1.5 rounded-full bg-emerald-50 hover:bg-emerald-100/80 border border-emerald-200 transition-all text-left"
                >
                  <div className="w-7 h-7 rounded-full bg-emerald-600 text-white font-bold text-xs flex items-center justify-center shadow-xs">
                    {currentUser.avatar || currentUser.name?.slice(0, 2).toUpperCase() || 'RQ'}
                  </div>
                ...
                {userDropdownOpen && (
                  <div className="absolute right-0 mt-2 w-64 glass-dropdown rounded-2xl shadow-card p-3 z-50 animate-slide-up border border-slate-200">
                    <div className="pb-2.5 mb-2 border-b border-slate-100">
                      ...
                      <div className="mt-1.5 inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-emerald-100 text-emerald-800 text-[10px] font-bold uppercase">
                    ...
                    <div className="space-y-1 text-xs">
                      <button
                        onClick={() => {
                          setUserDropdownOpen(false);
                          if (currentUser.role === 'seller') handleLinkClick('/seller');
                          else if (currentUser.role === 'ngo') handleLinkClick('/ngo');
                          else handleLinkClick('/browse');
                        }}
                        className="w-full flex items-center gap-2.5 px-2.5 py-2 rounded-xl text-slate-700 hover:bg-slate-100 transition-all text-left"
                      >
                        <Sparkles className="w-4 h-4 text-emerald-600" />
                        <span>My Persona Dashboard</span>
                      </button>

                      <button
                        onClick={() => {
                          setUserDropdownOpen(false);
                          handleLinkClick('/order-confirmation/order-demo');
                        }}
                        className="w-full flex items-center gap-2.5 px-2.5 py-2 rounded-xl text-slate-700 hover:bg-slate-100 transition-all text-left"
                      >
                    ...
                    <div className="pt-2 mt-2 border-t border-slate-100">
                      <button
                        ...
                        className="w-full flex items-center gap-2 px-2.5 py-2 rounded-xl text-xs font-semibold text-rose-600 hover:bg-rose-50 transition-all"
                      >
            ...
            {isAuthenticated && currentUser ? (
              <div className="space-y-2.5">
                <div className="flex items-center gap-3 p-2.5 bg-slate-50 rounded-xl border border-slate-100">
                  ...
                  <span className="text-[10px] font-bold uppercase bg-emerald-100 text-emerald-800 px-2 py-0.5 rounded-full shrink-0">
                    {currentUser.role}
                  </span>
                </div>
                ...
                <button
                  ...
                  className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl bg-rose-50 border border-rose-200 text-rose-700 text-xs font-bold hover:bg-rose-100 transition-colors"
                >
              ...
              <button
                onClick={() => handleLinkClick('/auth')}
                className="w-full py-2.5 rounded-xl bg-emerald-600 text-white text-xs font-bold text-center flex items-center justify-center gap-1.5 shadow-sm"
              >
                <User className="w-3.5 h-3.5" />
                <span>Sign In to ResQFood</span>
              </button>
            )}
          </div>
        </div>
